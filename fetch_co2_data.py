...
# -------- Configuration -------- #
API_KEY = "bcef3933b7664d0588160ab56ad969d3"
...
DATE_FMT = "%d-%m-%Y"
OUTPUT_EXCEL = "co2_data.xlsx"

# -------------------------------- #

def _call_api(from_date: str, to_date: str) -> List[Dict]:
    """Send a POST request to the CO2 API and return the parsed JSON response."""
...
def _flatten_response(raw: List[Dict]) -> pd.DataFrame:
    ...
    elif isinstance(raw, dict) and "co2" in raw:
        data_list = raw["co2"] or []
    ...
    for day_entry in data_list:
        ...
        data_field = day_entry.get("co2Data") or day_entry.get("data") or []
...
def fetch_co2_data(from_date: datetime.date, to_date: datetime.date) -> pd.DataFrame:
    """Fetch CO2 data between two dates and return as DataFrame."""
...
def main() -> None:
    ...
    print(f"Fetching CO2 data for {yesterday.strftime(DATE_FMT)} …")
    recent_df = fetch_co2_data(yesterday, yesterday)

    # 2. Fetch data from 01-Jan-2021 to yesterday
    start_date = datetime.date(2021, 1, 1)
    print(f"Fetching CO2 history from {start_date.strftime(DATE_FMT)} to {yesterday.strftime(DATE_FMT)} …")
    history_df = fetch_co2_data(start_date, yesterday)
    ...
    print(f"Done! CO2 data written to {OUTPUT_EXCEL}.")
