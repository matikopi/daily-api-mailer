...
# -------- Configuration -------- #
API_KEY = "17f0ec820d8f47a1a261a8abcb111db3"
...
DATE_FMT = "%d-%m-%Y"
OUTPUT_EXCEL = "smp_data.xlsx"

# -------------------------------- #

def _call_api(from_date: str, to_date: str) -> List[Dict]:
    """Send a POST request to the SMP API and return the parsed JSON response."""
...
def _flatten_response(raw: List[Dict]) -> pd.DataFrame:
    ...
    elif isinstance(raw, dict) and "smp" in raw:
        data_list = raw["smp"] or []
    ...
    for day_entry in data_list:
        ...
        data_field = day_entry.get("smpData") or day_entry.get("data") or []
...
def fetch_smp_data(from_date: datetime.date, to_date: datetime.date) -> pd.DataFrame:
    """Fetch SMP data between two dates and return as DataFrame."""
...
def main() -> None:
    ...
    print(f"Fetching SMP data for {yesterday.strftime(DATE_FMT)} …")
    recent_df = fetch_smp_data(yesterday, yesterday)

    # 2. Fetch data from 01-Jan-2021 to yesterday
    start_date = datetime.date(2021, 1, 1)
    print(f"Fetching SMP history from {start_date.strftime(DATE_FMT)} to {yesterday.strftime(DATE_FMT)} …")
    history_df = fetch_smp_data(start_date, yesterday)
    ...
    print(f"Done! SMP data written to {OUTPUT_EXCEL}.")
